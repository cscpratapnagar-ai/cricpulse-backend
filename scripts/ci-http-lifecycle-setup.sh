...
TEAM_A="$(curl -fsS -X POST "$BASE_URL/teams" "${AUTH[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg n "CricPulse CI A ${RUN_KEY}" '{name:$n,city:"CI"}')")"
TEAM_B="$(curl -fsS -X POST "$BASE_URL/teams" "${AUTH[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg n "CricPulse CI B ${RUN_KEY}" '{name:$n,city:"CI"}')")"
TEAM_A_ID="$(jq -r '.id' <<<"$TEAM_A")"
TEAM_B_ID="$(jq -r '.id' <<<"$TEAM_B")"
test -n "$TEAM_A_ID" && test -n "$TEAM_B_ID"

for team_id in "$TEAM_A_ID" "$TEAM_B_ID"; do
  curl -fsS -X POST "$BASE_URL/teams/$team_id/members" "${AUTH[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg p "$PLAYER_ID" '{playerId:$p,role:"PLAYER"}')" >/dev/null
done

MATCH="$(curl -fsS -X POST "$BASE_URL/matches" "${AUTH[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg n "CricPulse CI Lifecycle ${RUN_KEY}" --arg a "$TEAM_A_ID" --arg b "$TEAM_B_ID" '{name:$n,teamAId:$a,teamBId:$b,format:"T20",totalOvers:20}')")"
