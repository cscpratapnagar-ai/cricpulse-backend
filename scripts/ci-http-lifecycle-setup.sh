...
login_token() {
  curl_json POST "$BASE_URL/auth/login" "$(jq -nc --arg e "$1" --arg p "$PASSWORD" '{email:$e,password:$p}')" | jq -r '.accessToken'
}

OWNER_A_TOKEN="$(login_token "$OWNER_A_EMAIL")"
OWNER_B_TOKEN="$(login_token "$OWNER_B_EMAIL")"
test -n "$OWNER_A_TOKEN" && test "$OWNER_A_TOKEN" != null
test -n "$OWNER_B_TOKEN" && test "$OWNER_B_TOKEN" != null
AUTH_A=(-H "Authorization: Bearer $OWNER_A_TOKEN")
AUTH_B=(-H "Authorization: Bearer $OWNER_B_TOKEN")

OWNER_A_PROFILE="$(curl -fsS "${AUTH_A[@]}" "$BASE_URL/players/me")"
PLAYER_A_LOGIN="$(login_token "$PLAYER_A_EMAIL")"
PLAYER_A_PROFILE="$(curl -fsS -H "Authorization: Bearer $PLAYER_A_LOGIN" "$BASE_URL/players/me")"
OWNER_B_PROFILE="$(curl -fsS "${AUTH_B[@]}" "$BASE_URL/players/me")"
PLAYER_B_LOGIN="$(login_token "$PLAYER_B_EMAIL")"
PLAYER_B_PROFILE="$(curl -fsS -H "Authorization: Bearer $PLAYER_B_LOGIN" "$BASE_URL/players/me")"

OWNER_A_PLAYER_ID="$(jq -r '.id' <<<"$OWNER_A_PROFILE")"
PLAYER_A_ID="$(jq -r '.id' <<<"$PLAYER_A_PROFILE")"
OWNER_B_PLAYER_ID="$(jq -r '.id' <<<"$OWNER_B_PROFILE")"
PLAYER_B_ID="$(jq -r '.id' <<<"$PLAYER_B_PROFILE")"

test -n "$OWNER_A_PLAYER_ID" && test "$OWNER_A_PLAYER_ID" != null
test -n "$PLAYER_A_ID" && test "$PLAYER_A_ID" != null
test -n "$OWNER_B_PLAYER_ID" && test "$OWNER_B_PLAYER_ID" != null
test -n "$PLAYER_B_ID" && test "$PLAYER_B_ID" != null
...
MATCH="$(curl -fsS -X POST "$BASE_URL/matches" "${AUTH_A[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg n "CricPulse CI Lifecycle ${RUN_KEY}" --arg a "$TEAM_A_ID" --arg b "$TEAM_B_ID" '{name:$n,teamAId:$a,teamBId:$b,format:"T20",totalOvers:20}')")"
...
select_xi() {
  ...
  local -n auth_ref="$auth_name"
  curl -fsS -X POST "$BASE_URL/matches/$MATCH_ID/playing-xi" "${auth_ref[@]}" -H 'Content-Type: application/json' --data "$(jq -nc --arg t "$team_id" --arg p "$player_id" '{teamId:$t,playerId:$p,captain:false,viceCaptain:false,wicketKeeper:false}')" >/dev/null
}

select_xi AUTH_A "$TEAM_A_ID" "$OWNER_A_PLAYER_ID"
select_xi AUTH_A "$TEAM_A_ID" "$PLAYER_A_ID"
select_xi AUTH_B "$TEAM_B_ID" "$OWNER_B_PLAYER_ID"
select_xi AUTH_B "$TEAM_B_ID" "$PLAYER_B_ID"
