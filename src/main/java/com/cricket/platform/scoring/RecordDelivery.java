    ...
    public DeliveryResponse execute(Request request){
        ...
        if(r.wicketType()!=null){
            ...
            if(newWickets>=10){if(r.dismissedPlayerId().equals(ns))ns=null;if(r.dismissedPlayerId().equals(nn))nn=null;}
                ...
                // Older innings could have the opening partnership stored as
                // wicket #1. Normalize that legacy row to wicket #0 before
                // creating the first post-wicket partnership (#1).
                if(newWickets==1){
                    jdbc.update("""
