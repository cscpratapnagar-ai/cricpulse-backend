...
public class RecordDelivery {
    ...
    public DeliveryResponse execute(Request request) {
        ...
        jdbc.update(
                ...
                wickets,
                legalBalls,
                legal ? legalBalls / 6 : previousOver,
                legal ? ((legalBalls - 1) % 6) + 1 : previousBall,
                request.bowlerId(),
                request.inningsId()
        );

        updateOverSummary(request, deliveryId, legal, totalRuns);
        ...
        if (request.wicketType() != null && request.newBatterId() != null) {
            ...
            } else if (request.dismissedPlayerId().equals(nextNonStriker)) {
                nextNonStriker = request.newBatterId();
            }
        }
    ...
    private void updateOverSummary(Request request, UUID deliveryId, boolean legal, int totalRuns) {
        int overNumber = request.overNumber();
        jdbc.update(
                """
    ...
    private void updateBatterStats(Request request, UUID deliveryId, boolean legal) {
        jdbc.update(
                ...
                request.strikerId(),
                request.batRuns(),
                legal && !"WIDE".equals(request.extraType()) && !"NO_BALL".equals(request.extraType()) ? 1 : 0,
                request.batRuns() == 4 ? 1 : 0,
                request.batRuns() == 6 ? 1 : 0,
                request.wicketType() != null && request.dismissedPlayerId().equals(request.strikerId()),
                request.wicketType(),
                request.wicketType() == null ? null : deliveryId
        );
    }
    ...
    private boolean isBowlerWicket(String wicketType) {
        return wicketType != null && Set.of("BOWLED", "CAUGHT", "LBW", "STUMPED", "HIT_WICKET").contains(wicketType);
    }

    private void validate(Request request) {
        if (request.batRuns() > 6) {
            throw new IllegalArgumentException("Bat runs cannot exceed 6");
        }
