...
public class GetScorecard {
    ...
    public List<Scorecard> executeAll(List<UUID> inningsIds) {
        ...
        jdbc.query("""
                ...
                WHERE i.id IN (""" + placeholders + ") ORDER BY i.innings_number",
                rs -> bases.put(rs.getObject("id", UUID.class), new Scorecard(
        ...
        jdbc.query("""
                SELECT b.innings_id, b.player_id, u.full_name, b.runs, b.balls_faced,
                       b.fours, b.sixes, b.strike_rate, b.is_out, b.dismissal_type
                FROM innings_batters b
                JOIN players p ON p.id = b.player_id
                JOIN users u ON u.id = p.user_id
                WHERE b.innings_id IN (""" + placeholders + ")
                ORDER BY b.innings_id, b.batting_position NULLS LAST, b.created_at",
                rs -> batting.computeIfAbsent(rs.getObject("innings_id", UUID.class), k -> new ArrayList<>())
        ...
        jdbc.query("""
                SELECT b.innings_id, b.player_id, u.full_name, b.legal_balls,
                       b.runs_conceded, b.wickets, b.economy
                FROM innings_bowlers b
                JOIN players p ON p.id = b.player_id
                JOIN users u ON u.id = p.user_id
                WHERE b.innings_id IN (""" + placeholders + ")
                ORDER BY b.innings_id, b.created_at",
                rs -> bowling.computeIfAbsent(rs.getObject("innings_id", UUID.class), k -> new ArrayList<>())
        ...
        jdbc.query("""
                SELECT f.innings_id, f.wicket_number, u.full_name, f.runs,
                       f.over_number, f.ball_number
                FROM fall_of_wickets f
                JOIN players p ON p.id = f.player_id
                JOIN users u ON u.id = p.user_id
                WHERE f.innings_id IN (""" + placeholders + ")
                ORDER BY f.innings_id, f.wicket_number",
                rs -> fow.computeIfAbsent(rs.getObject("innings_id", UUID.class), k -> new ArrayList<>())
        ...
        jdbc.query("""
                SELECT innings_id, COALESCE(SUM(extra_runs), 0) AS extras
                FROM deliveries WHERE innings_id IN (""" + placeholders + ")
                GROUP BY innings_id""",
