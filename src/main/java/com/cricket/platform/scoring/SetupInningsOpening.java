...
public class SetupInningsOpening {
    ...
    public OpeningResponse execute(Request request) {
        ...
        return new OpeningResponse(
                request.inningsId(), request.strikerId(), request.nonStrikerId(),
                request.bowlerId(), "READY_FOR_FIRST_BALL");
    }
    ...
    private void requirePlayingPlayer(UUID inningsId, UUID teamId, UUID playerId) {
        Integer count = jdbc.queryForObject(
                ...
                WHERE i.id = ?
                """,
                Integer.class, teamId, playerId, inningsId);
    ...
    private record InningsState(
            UUID battingTeamId, UUID bowlingTeamId, String status,
            UUID strikerId, UUID nonStrikerId, UUID currentBowlerId
    ) {}

    public record Request(
            @NotNull UUID inningsId, @NotNull UUID strikerId,
            @NotNull UUID nonStrikerId, @NotNull UUID bowlerId
    ) {}

    public record OpeningResponse(
            UUID inningsId, UUID strikerId, UUID nonStrikerId,
            UUID bowlerId, String status
    ) {}
}
