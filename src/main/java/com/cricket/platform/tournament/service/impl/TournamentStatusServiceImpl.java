...
public class TournamentStatusServiceImpl implements TournamentStatusService {
    ...
    @Override
    @Transactional
    public TournamentResponse changeStatus(UUID tournamentId, ChangeTournamentStatusRequest request,
                                           Authentication authentication) {
        requireOwner(tournamentId, authentication);
        String currentStatus = jdbc.queryForObject("SELECT status FROM tournaments WHERE id=?", String.class, tournamentId);
        ...
        jdbc.update("UPDATE tournaments SET status=? WHERE id=?", targetStatus, tournamentId);
    ...
    private void validateTransition(String current, String target, UUID tournamentId) {
        if (current == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Tournament not found");
        if (current.equals(target)) throw new ResponseStatusException(HttpStatus.CONFLICT, "Tournament is already " + target);

        if ("DRAFT".equals(current) && "ACTIVE".equals(target)) {
            Integer teamCount = jdbc.queryForObject("SELECT COUNT(*) FROM tournament_teams WHERE tournament_id=?", Integer.class, tournamentId);
            Integer fixtureCount = jdbc.queryForObject("SELECT COUNT(*) FROM tournament_matches WHERE tournament_id=?", Integer.class, tournamentId);
            if (teamCount == null || teamCount < 2)
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "At least 2 teams are required before activating the tournament");
            if (fixtureCount == null || fixtureCount < 1)
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Generate or add at least 1 fixture before activating the tournament");
            return;
        }

        if ("ACTIVE".equals(current) && "COMPLETED".equals(target)) {
            Integer totalFixtures = jdbc.queryForObject("SELECT COUNT(*) FROM tournament_matches WHERE tournament_id=?", Integer.class, tournamentId);
            Integer completedFixtures = jdbc.queryForObject(
                    "SELECT COUNT(*) FROM tournament_matches tm JOIN matches m ON m.id=tm.match_id WHERE tm.tournament_id=? AND m.status='COMPLETED'",
                    Integer.class, tournamentId);
            if (totalFixtures == null || totalFixtures < 1)
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Tournament has no fixtures to complete");
            if (!Objects.equals(totalFixtures, completedFixtures))
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "All tournament fixtures must be completed before completing the tournament");
            return;
        }
