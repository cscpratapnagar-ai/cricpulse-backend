...
public class TournamentStandingsServiceImpl implements TournamentStandingsService {
    ...
    @Override
    public List<TournamentController.PointRow> getPointsTable(UUID tournamentId, Authentication authentication) {
        requireOwner(tournamentId, authentication);

        List<TournamentController.PointRow> standings = new ArrayList<>();

        List<TournamentController.TeamView> teams = jdbc.query(
                ...
                (row, index) -> new TournamentController.TeamView(
        ...
        for (TournamentController.TeamView team : teams) {
            int played = 0;
            int wins = 0;
            int losses = 0;
            int ties = 0;
            ...
            List<MatchScore> matches = jdbc.query(
                    "SELECT m.id,ia.batting_team_id ia_team,ia.total_runs ia_runs,ia.legal_balls ia_balls," +
                            "ib.batting_team_id ib_team,ib.total_runs ib_runs,ib.legal_balls ib_balls " +
                            "FROM tournament_matches tm " +
                            "JOIN matches m ON m.id=tm.match_id " +
                            "LEFT JOIN innings ia ON ia.match_id=m.id AND ia.innings_number=1 " +
                            "LEFT JOIN innings ib ON ib.match_id=m.id AND ib.innings_number=2 " +
                            "WHERE tm.tournament_id=? AND m.status='COMPLETED' " +
                            "AND (m.team_a_id=? OR m.team_b_id=?)",
                    (row, index) -> new MatchScore(
                            row.getObject("id", UUID.class),
                            row.getObject("ia_team", UUID.class),
                            row.getInt("ia_runs"),
                            row.getInt("ia_balls"),
                            row.getObject("ib_team", UUID.class),
                            row.getInt("ib_runs"),
                            row.getInt("ib_balls")),
            ...
            standings.add(new TournamentController.PointRow(
                    team.id(), team.name(), played, wins, losses, ties,
                    points, runsFor, runsAgainst, nrr));
        }

        standings.sort(
                Comparator.comparingInt(TournamentController.PointRow::points).reversed()
                        .thenComparing(Comparator.comparingDouble(TournamentController.PointRow::nrr).reversed())
                        .thenComparing(Comparator.comparingInt(TournamentController.PointRow::runsFor).reversed())
                        .thenComparing(TournamentController.PointRow::teamName));

        return standings;
    }

    @Override
    public TournamentController.QualificationPreview getQualificationPreview(
            UUID tournamentId, Authentication authentication) {

        List<TournamentController.PointRow> table = getPointsTable(tournamentId, authentication);

        if (table.size() < 4) {
            return new TournamentController.QualificationPreview(
                    false,
                    "At least 4 tournament teams are required for a top-4 qualification bracket.",
                    table,
                    List.of());
        }

        List<TournamentController.Qualifier> qualifiers = List.of(
                new TournamentController.Qualifier(1, "QUALIFIER 1", table.get(0), table.get(3)),
                new TournamentController.Qualifier(2, "QUALIFIER 2", table.get(1), table.get(2)));
    ...
    private record MatchScore(
            UUID matchId,
            UUID firstBattingTeam,
            int firstRuns,
            int firstBalls,
            UUID secondBattingTeam,
            int secondRuns,
            int secondBalls) {
    }
}