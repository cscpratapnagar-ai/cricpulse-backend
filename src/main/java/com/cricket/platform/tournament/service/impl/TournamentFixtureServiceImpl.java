...
public class TournamentFixtureServiceImpl implements TournamentFixtureService {
    ...
    @Override
    public TournamentController.FixtureView addMatch(UUID tournamentId, UUID matchId, String stage,
        ...
        if (validTeams == null || validTeams == 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Both match teams must belong to the tournament");
        }
        try {
            jdbc.update(
    ...
    @Override
    @Transactional
    public TournamentController.GenerateFixturesResponse generateFixtures(UUID tournamentId,
                                                                            Authentication authentication) {
        requireOwner(tournamentId, authentication);
        ensureEditable(tournamentId);
        ...
        jdbc.query(
                ...
                row -> existingPairs.add(pairKey(row.getObject("team_a_id", UUID.class),
                        row.getObject("team_b_id", UUID.class))), tournamentId);

        Integer maxFixture = jdbc.queryForObject(
                "SELECT MAX(fixture_number) FROM tournament_matches WHERE tournament_id=?",
        ...
        for (int i = 0; i < teams.size(); i++) {
            for (int j = i + 1; j < teams.size(); j++) {
                TournamentController.TeamView home = teams.get(i);
                TournamentController.TeamView away = teams.get(j);
        ...
        return new TournamentController.GenerateFixturesResponse(
                tournamentId, generated, skipped, generated + skipped, findFixtures(tournamentId, authentication));
    }

    @Override
    public List<TournamentController.FixtureView> findFixtures(UUID tournamentId, Authentication authentication) {
        requireOwner(tournamentId, authentication);
        return jdbc.query(
                ...
                (row, index) -> fixtureView(row), tournamentId);
    }

    @Override
    public TournamentController.FixtureView scheduleFixture(UUID tournamentId, UUID matchId,
        ...
        if (request.scheduledAt().isBefore(OffsetDateTime.now().minusMinutes(1))) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Fixture time must be in the future");
        }
        ...
        if (linked == null || linked == 0) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Fixture is not linked to this tournament");
        }
    ...
    private TournamentController.FixtureView findFixture(UUID tournamentId, UUID matchId) {
        return jdbc.queryForObject(
                "SELECT tm.match_id,tm.fixture_number,tm.stage,m.name,m.team_a_id,x.name team_a_name,m.team_b_id,y.name team_b_name,m.status,m.scheduled_at FROM tournament_matches tm JOIN matches m ON m.id=tm.match_id JOIN teams x ON x.id=m.team_a_id JOIN teams y ON y.id=m.team_b_id WHERE tm.tournament_id=? AND tm.match_id=?",
                (row, index) -> fixtureView(row), tournamentId, matchId);
    }

    private TournamentController.FixtureView fixtureView(java.sql.ResultSet row) throws java.sql.SQLException {
        Object scheduledAt = row.getObject("scheduled_at");
        return new TournamentController.FixtureView(
                row.getObject("match_id", UUID.class),
                row.getObject("fixture_number", Integer.class),
                row.getString("stage"),
                row.getString("name"),
                row.getObject("team_a_id", UUID.class),
                row.getString("team_a_name"),
                row.getObject("team_b_id", UUID.class),
                row.getString("team_b_name"),
                row.getString("status"),
                scheduledAt == null ? null : scheduledAt.toString());
    }

    private void ensureEditable(UUID tournamentId) {
        String status = jdbc.queryForObject("SELECT status FROM tournaments WHERE id=?",
