...
echo "============================================================"
echo " CRICPULSE RUN_OUT + 5 RUNS + NEW BATTER E2E - REPOSITORY VERIFIED"
echo "============================================================"
echo
...
if [[ "$RECORD_HTTP" != "200" ]]; then echo "[FAIL] Record RUN_OUT + 5 runs (HTTP $RECORD_HTTP)" >&2; echo "$RECORD_JSON" >&2; exit 1; fi
echo "[PASS] Record RUN_OUT + 5 runs (HTTP 200)"
...
[[ "$AFTER_RUNS" -eq $((RUNS + BAT_RUNS)) ]] && echo "[PASS] Team score increased by 5 runs" || { echo "[FAIL] Team score mismatch after RUN_OUT + 5 runs" >&2; exit 1; }
# A 4 is an # Odd completed runs swap ends. If this delivery completes an over, the normal over-end swap applies once more.
if (( AFTER_LEGAL % 6 == 0 )); then
  [[ "$AFTER_STRIKER" == "$STRIKER" ]] && echo "[PASS] Original striker correctly returns to striker end after over-end swap" || { echo "[FAIL] Original striker position incorrect after over-end swap" >&2; exit 1; }
  [[ "$AFTER_NON" == "$NEW_BATTER" ]] && echo "[PASS] New batter correctly moves to non-striker end after over-end swap" || { echo "[FAIL] New batter position incorrect after over-end swap" >&2; exit 1; }
else
  ...
  [[ "$AFTER_NON" == "$STRIKER" ]] && echo "[PASS] Original striker correctly moves to non-striker end after odd runs" || { echo "[FAIL] Original striker position incorrect after odd runs" >&2; exit 1; }
fi
...
echo
echo "============================================================"
echo " RUN_OUT + 5 RUNS + NEW BATTER E2E PASSED"
echo "============================================================"
echo
