...
# CricPulse OBSTRUCTING_THE_FIELD + NEW BATTER E2E.
# Keeps the existing authentication/scoring flow unchanged.
# The Playing XI read uses the innings compatibility endpoint first and
# falls back to the canonical match Playing XI endpoint when the running
# backend does not expose the compatibility route yet.
#
...
echo "============================================================"
echo " CRICPULSE OBSTRUCTING_THE_FIELD + NEW BATTER E2E - REPOSITORY VERIFIED"
echo "============================================================"
echo
...
[[ -n "$MATCH_ID" ]] && echo "    match       : $MATCH_ID"
echo "    status      : $STATUS"
...
DELIVERY_BODY="$(jq -nc \
  ...
  --argjson over "$OVER_NUMBER" \
  --argjson ball "$BALL_NUMBER" \
  '{inningsId:$innings,overNumber:$over,ballNumber:$ball,strikerId:$striker,nonStrikerId:$non,bowlerId:$bowler,batRuns:0,extraRuns:0,extraType:null,wicketType:$wicket,dismissedPlayerId:$dismissed,newBatterId:$newbat}')"
...
if [[ "$RECORD_HTTP" != "200" ]]; then
  echo "[FAIL] Record OBSTRUCTING_THE_FIELD wicket (HTTP $RECORD_HTTP)" >&2
  echo "$RECORD_JSON" >&2
  exit 1
fi
echo "[PASS] Record OBSTRUCTING_THE_FIELD wicket (HTTP 200)"
...
[[ "$AFTER_WICKETS" -eq $((WICKETS + 1)) ]] && echo "[PASS] OBSTRUCTING_THE_FIELD wicket count +1" || { echo "[FAIL] Wicket count did not increment" >&2; exit 1; }
...
BATTER="$(jq -c --arg id "$STRIKER" '.batters[]? | select((.playerId // .player_id) == $id)' <<<"$AFTER" | head -n1)"
...
[[ "$DISMISSAL" == "$WICKET_TYPE" ]] && echo "[PASS] Dismissed batter marked OBSTRUCTING_THE_FIELD" || { echo "[FAIL] Dismissed batter dismissal type mismatch" >&2; exit 1; }
[[ "$FOW_COUNT" -ge $((WICKETS + 1)) ]] && echo "[PASS] Fall of wicket recorded" || { echo "[FAIL] Fall of wicket not recorded" >&2; exit 1; }
[[ -n "$RECENT_WICKET" ]] && echo "[PASS] Recent delivery contains OBSTRUCTING_THE_FIELD wicket" || { echo "[FAIL] Recent delivery missing OBSTRUCTING_THE_FIELD" >&2; exit 1; }

echo
echo "============================================================"
echo " OBSTRUCTING_THE_FIELD + NEW BATTER E2E PASSED"
echo "============================================================"
echo
